...
const MapScaleBologna = ({
  definitionLayer,
  top,
}: MapScaleProps): JSX.Element => {
  const zoom = useAppSelector((state) => state.bologna.zoom);
  const latitude = useAppSelector((state) => state.bologna.latitude);
  const minZoom = useAppSelector((state) => state.bologna.minZoom);
  const maxZoom = useAppSelector((state) => state.bologna.maxZoom);
  const maxWidth = 100; // of the scale bar
  const pitch = useAppSelector((state) => state.bologna.pitch);
...
export default MapScaleBologna;
